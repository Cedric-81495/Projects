...
- [x] **Responsive audit, 280-1920px — seven defects fixed.** See
      ...
      buttons were two-up on every phone, shredding "Business / Entrepreneurship"
      across four lines on the event page's key control. Added a `min-width:0`
      safety net on all variable-content containers — the root cause of most
      sideways scrolling. Breakpoints cut from 15 to 12 and documented; the
      three-query hero split collapsed to one.
