...
export const funnel = {
  ...
  /* ⚠ THE `{plan}` TOKEN IS GONE, 2026-09-14. It existed to render ", with a
     3-payment plan available" from BLUEPRINT_BUNDLE.monthly, which no longer
     exists — the Pricing, Payment & Package Master rules out instalments and
     subscriptions entirely.

     History, so nobody re-adds it: the clause was hardcoded plain text until
     2026-09-11, which meant that when the plan was withdrawn from the bundle
     card it survived here — the offer gone from one surface and still promised
     on another, in the answer to "What does it cost?". Tokenising it fixed
     that; removing the plan makes the token dead.

     Still do not write a price or a payment term into this string directly.
     Tokens are what keep the FAQ from disagreeing with the card above it. */
  costAnswer:
    ...
    + 'savings). {refund} so we\'ll walk through '
