          ...
          <ul className="divide-y divide-faint/30 border-y border-faint/30">
            {all.map((ep, i) => (
              <Reveal as="li" key={`${ep.id}-${i}`} delay={(i % 3) * 60}>
                <button
                  type="button"
                  className="group flex w-full items-center gap-5 py-6 text-left transition-colors hover:bg-raise/40"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline gap-3">
                      <span className="font-mono text-xs text-gold/70">{ep.number}</span>
                      <span className="truncate text-xs text-muted">{ep.guest}</span>
                    </div>
